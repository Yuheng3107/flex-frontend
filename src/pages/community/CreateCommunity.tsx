...
function CreateCommunity() {
  ...
  async function submitHandler(event: React.FormEvent) {
    ...
    if (
      ...
      communityDescriptionInput.trim() === ""
    )
      return presentToast(
        "Please enter a name and description for your community"
      );
    else if (communityNameInput.trim() === "")
      return presentToast("Please enter a name for your community");
    else if (communityDescriptionInput.trim() === "")
      return presentToast("Please enter a description for your community");
    ...
    if (
      ...
      bannerInputRef.current!.files !== null &&
      bannerInputRef.current!.files[0] !== undefined
    ) {
      //type guard
      bannerFormData.append("media", bannerInputRef.current!.files[0]);
    }

    if (imageFormData.has("media") === false)
      return presentToast("Please upload a community photo");
    if (bannerFormData.has("media") === false)
      return presentToast("Please upload a banner photo");
    let response = await createCommunityAsync(
      communityNameInput,
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonButtons slot="end">
            <IonButton
              className="mr-1 rounded-lg bg-sky-400 py-2 px-3 text-white"
