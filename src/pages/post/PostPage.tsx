...
const PostPage: React.FC<PostPageProps> = ({ match }) => {
  ...
  const [communityData, setCommunityData] =
    useState<CommunityData>(emptyCommunityData);
  ...
  const loadComments = async () => {
    ...
    let commentArray = await getCommentsAsync(
      postData.comments.slice(start, end)
    );
  ...
  const submitHandler = async () => {
    ...
    let response = await createCommentAsync(
      "user",
      Number(match.params.postId),
      "",
      postTextInput
    );
